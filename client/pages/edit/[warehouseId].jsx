...
function EditWarehouse() {
  ...
  const getData = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_DB_LINK}/api/warehouse/${warehouseId}`
      );
  ...
  const edithandler = async (e) => {
    try {
      ...
      await axios
        .put(
          `${process.env.NEXT_PUBLIC_DB_LINK}/api/warehouse/update/${router.query.warehouseId}`,
          data
        )
        .then(() => {
            ...
            <Stack spacing={4}>
              <form onSubmit={edithandler}>
                <FormControl isInvalid={errors ? errors.name : null}>
                  ...
                  />
                  {imgId ? <Text>Already an image exists</Text> : null}
                  <FormErrorMessage>
